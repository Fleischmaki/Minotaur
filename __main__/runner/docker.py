...
import time
import os, sys
import Fuzzle.runner.commands as commands
...
def spawn_cmd_in_docker(container, cmd_str):
    print('[*] Executing (in %s): %s' % (container, cmd_str))
...
def copy_docker_output(tool, name, out_path, user):
    return commands.run_cmd(CP_CMD % (get_container(tool,name), user, out_path if not os.path.isdir(out_path) else os.path.join(out_path,'.')))
