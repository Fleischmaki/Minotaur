...
def signed(node: FNode,converted_node: str, always=True) -> str:
    ...
    if not always and width in (32,64) and (len(node.args()) == 0 or (all(map(is_signed, node.args())) and all(map(lambda n: ff.get_bv_width(n)<=width,node.args())))):
...
def div_helper(symbs: t.Set[str],node: FNode, cons: io.TextIOBase):
    ...
    lString = cast(node,convert_to_string(symbs, l))
    rString = cast(node,convert_to_string(symbs, r))
...
def convert(symbs: t.Set[str],node: FNode,cons: io.TextIOBase):
    ...
    if node.is_iff() or node.is_equals() or node.is_bv_comp():
        ...
        cons.write(unsigned(node,base) + ' - ' + '(' + ucast)
        ...
        cons.write(cast(node,var) if dim == 0 else var)
        symbs.add(var)
    elif node.is_select():
        (a, p) = node.args()
        ...
        if dim == 1:
            ...
            convert(symbs,p,cons)
            cons.write(")")
        if 'BV' in str(node.get_type()) and ff.get_bv_width(node) not in (8,16,32,64): 
            cons.write(')')
    elif node.is_store():
        (a, p, v) = node.args()
