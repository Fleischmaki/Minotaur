...
GEN_CMD = './generate.sh -o %s -a %s -w %s -h %s -r %s -n %s -c %s -b %s -g %s -s %s'
...
def generate(mazes, out_dir):
    for maze in mazes:
        ...
        if 'CVE' in tokens[7]:
            gen = 'CVE_gen'
            smt = "../CVEs/" + tokens[7].split('_')[0] + ".smt2"
        else:
            gen = tokens[7]
            smt = "default"    
        cmd = GEN_CMD % (out_dir, algo, width, height, seed, numb, cycle, bugtype, gen, smt)
        run_cmd(cmd)
